...
def verificar_claves_api(api_key, secret_key):
    try:
        if secret_key.startswith("Kraken"):
            ...
            ws = create_connection(api_domain)(key=api_key, secret=kraken_secret)
            create_connection.load_key('kraken.key')
            return True
        else:
            create_connection = Client(api_key, secret_key)
            return True
    except:
        return False
...
def verificar_disponibilidad_dinero(api_key, secret_key, cantidad):
    if secret_key.startswith("Kraken"):
        kraken_secret = secret_key.split(":")[1]
        api = create_connection(key=api_key, secret=kraken_secret)
        api.load_key('kraken.key')
        balance = api.query_private('Balance')
        free_balance = balance['result']['ZUSD']
    else:
...
def crear_archivo_trading():
    ...
    df = df.DataFrame({'Fecha': [now], 'Cantidad': [], 'Precio': []})
...
class TradingApp(QMainWindow):
    ...
    def validate_keys(self, api_key, secret_key):
        ...
        if self.exchange == "Binance":
            ...
            client = create_connection(key=api_key, secret=secret_key)
    ...
    def chart_update(self):
        ...
        if self.exchange == "Binance":
            ...
            ws = create_connection("wss://ws.kraken.com/")
            ws.send('{"event":"subscribe", "pair":["%(symbol)s"], "subscription":{"name":"ohlc"}}' % {"symbol": symbol.lower()})
            response = ws.recv()
            ws.close()
...
class ValidationDialog(QDialog):
    ...
    def accept(self):
        ...
        parent = self.parent()
        create_connection(api_key, secret_key)
        super().accept()
