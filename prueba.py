...
def verificar_claves_api(api_key, secret_key):
    try:
        if secret_key.startswith("Kraken"):
            ...
            api = krakenex.API(key=api_key, secret=kraken_secret)
...
class TradingApp(QMainWindow):
    ...
    def validate_keys(self, api_key, secret_key):
        ...
        if self.exchange == "Binance":
            ...
            self.client = krakenex.API(key=api_key, secret=secret_key)
    ...
    def chart_update(self):
        ...
        if self.exchange == "Binance":
            ...
            klines = self.client.query_public('OHLC', {'pair': symbol, 'interval': self.time_interval})
        ...
        if self.graph_window.figure is not None:
            ...
            if self.exchange == "Binance":
                ...
                for entry in klines['result'][symbol]:
                    timestamp = entry[0]
                    close_price = float(entry[4])
                    open_price = float(entry[1])
                    high_price = float(entry[2])
