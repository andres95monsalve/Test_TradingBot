...
from PyQt5.QtWidgets import (
    QApplication, QComboBox, QDesktopWidget, QDialog,
...
class TradingApp(QMainWindow):
    ...
    def validate_keys(self, api_key, secret_key):
        if not verificar_claves_api(api_key, secret_key, self.exchange):
    ...
    def chart_update(self):
        ...
        if self.exchange == "Binance":
            ...
            ws = create_connection("wss://ws.kraken.com/")
            ws.send(f'{{"event": "ohlc", "pair": ["{symbol}"], "interval": 1}}')
            response = ws.recv()
            ws.close()
            klines = response.split('\n')[:-1]

        self.graph_window.ax.clear()
        ...
        if self.exchange == "Binance":
            ...
            for entry in klines:
                if entry.startswith('['):
                    symbol = 'BTCUSDT'
                    btc_prices = self.client.get_historical_klines(symbol, Client.KLINE_INTERVAL_1DAY)
                    symbol = 'ETHUSDT'
                    eth_prices = self.client.get_historical_klines(symbol, Client.KLINE_INTERVAL_1DAY)
                    symbol = 'LTCUSDT'
                    ltc_prices = self.client.get_historical_klines(symbol, Client.KLINE_INTERVAL_1DAY)

                    btc_closes = [float(price[4]) for price in btc_prices]
                    eth_closes = [float(price[4]) for price in eth_prices]
                    ltc_closes = [float(price[4]) for price in ltc_prices]

                    ohlc.append([btc_closes, eth_closes, ltc_closes])
        ...
        self.graph_window.ax.xaxis.set_major_formatter(plt.FuncFormatter(lambda timestamp, _: datetime.datetime.fromtimestamp(timestamp/1000).strftime('%H:%M')))
        self.graph_window.ax.xaxis.set_minor_formatter(plt.FuncFormatter(lambda timestamp, _: datetime.datetime.fromtimestamp(timestamp/1000).strftime('%d/%m/%Y')))
