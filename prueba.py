...
class TradingApp(QMainWindow):
    ...
    def validate_keys(self, api_key, secret_key):
        if verificar_claves_api(api_key, secret_key, self.exchange):
            ...
            self.secret_key = secret_key.replace(" ", "")
            self.validado = True
    ...
    def handle_combo_box_change(self, coin):
        if self.exchange == "Binance":
            ...
            if coin == "BTC":
                ...
                self.kraken_var = "XETCZUSD"
            elif coin == "LTC":
                self.kraken_var = "LTCUSDT"
    ...
    def chart_update(self):
        ...
        if self.exchange == "Binance":
            ...
            data, last = kraken.get_ohlc_data("XXBTZUSD")
            data.index = date2num(data.index.to_pydatetime())

        ohlc = []
        if self.exchange == "Binance":
            
            api_url = "https://api.cryptowat.ch/markets/BINANCE/BTC-USDT/ohlc"
            params = {
                'periods': '3600',
                'after': str(int((time.time() - 3600 * 24 * 7) // 3600 * 3600))
            }
            ...
            df['ClosePrice'].plot(kind='line', title='Recent price evolution')
            plt.show()
